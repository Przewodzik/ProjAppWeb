<?php


session_start(); // Inicjalizacja sesji
ob_start(); // Włączenie buforowania wyjścia
include('includes/admin-head.html'); // Wczytanie nagłówka
include('cfg.php'); // Wczytanie pliku konfiguracyjnego

// Sprawdzenie, czy użytkownik jest zalogowany
if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true) {
    // Jeśli użytkownik nie jest zalogowany, przekieruj na stronę logowania
    header('Location: admin.php');
    exit;
}
// Obsługa wylogowania
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['logout'])) {
    session_unset();
    session_destroy();
    header('Location: admin.php'); // Przekierowanie na stronę logowania
    exit;
}

// Menu panelu administracyjnego
echo '
    <div class="menu">
        <h1>Panel Administracyjny</h1>
    </div>
    <nav>
         <div class="icon-text">
            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M620-163 450-333l56-56 114 114 226-226 56 56-282 282Zm220-397h-80v-200h-80v120H280v-120h-80v560h240v80H200q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h167q11-35 43-57.5t70-22.5q40 0 71.5 22.5T594-840h166q33 0 56.5 23.5T840-760v200ZM480-760q17 0 28.5-11.5T520-800q0-17-11.5-28.5T480-840q-17 0-28.5 11.5T440-800q0 17 11.5 28.5T480-760Z"/></svg>
            <button type="button" class="admin-button" onclick="window.location.href=\'products.php\'">Produkty</button>
        </div>
        <div class="icon-text">
            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="m260-520 220-360 220 360H260ZM700-80q-75 0-127.5-52.5T520-260q0-75 52.5-127.5T700-440q75 0 127.5 52.5T880-260q0 75-52.5 127.5T700-80Zm-580-20v-320h320v320H120Zm580-60q42 0 71-29t29-71q0-42-29-71t-71-29q-42 0-71 29t-29 71q0 42 29 71t71 29Zm-500-20h160v-160H200v160Zm202-420h156l-78-126-78 126Zm78 0ZM360-340Zm340 80Z"/></svg>
            <button type="button" class="admin-button" onclick="window.location.href=\'categories.php\'">Kategorie</>
        </div>
        <div class="icon-text">
            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M640-160v-360H160v360h480Zm80-200v-80h80v-360H320v200h-80v-200q0-33 23.5-56.5T320-880h480q33 0 56.5 23.5T880-800v360q0 33-23.5 56.5T800-360h-80ZM160-80q-33 0-56.5-23.5T80-160v-360q0-33 23.5-56.5T160-600h480q33 0 56.5 23.5T720-520v360q0 33-23.5 56.5T640-80H160Zm400-603ZM400-340Z"/></svg>
            <button type="button" class="admin-button" onclick="window.location.href=\'pages.php\'">Strony</button>
        </div>
        <div class="icon-text">
            <form method="post">
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h280v80H200v560h280v80H200Zm440-160-55-58 102-102H360v-80h327L585-622l55-58 200 200-200 200Z"/></svg>
                <button type="submit" name="logout" class="logout-button">Wyloguj się</button>
            </form>
        </div>
        <button id="theme-switch">
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368">
                    <path d="M480-120q-150 0-255-105T120-480q0-150 105-255t255-105q14 0 27.5 1t26.5 3q-41 29-65.5 75.5T444-660q0 90 63 153t153 63q55 0 101-24.5t75-65.5q2 13 3 26.5t1 27.5q0 150-105 255T480-120Z"/>
                </svg>
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368">
                    <path d="M480-280q-83 0-141.5-58.5T280-480q0-83 58.5-141.5T480-680q83 0 141.5 58.5T680-480q0 83-58.5 141.5T480-280ZM200-440H40v-80h160v80Zm720 0H760v-80h160v80ZM440-760v-160h80v160h-80Zm0 720v-160h80v160h-80ZM256-650l-101-97 57-59 96 100-52 56Zm492 496-97-101 53-55 101 97-57 59Zm-98-550 97-101 59 57-100 96-56-52ZM154-212l101-97 55 53-97 101-59-57Z"/>
                </svg>
            </button>
            <div class="icon-text">
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M360-840v-80h240v80H360Zm80 440h80v-240h-80v240Zm40 320q-74 0-139.5-28.5T226-186q-49-49-77.5-114.5T120-440q0-74 28.5-139.5T226-694q49-49 114.5-77.5T480-800q62 0 119 20t107 58l56-56 56 56-56 56q38 50 58 107t20 119q0 74-28.5 139.5T734-186q-49 49-114.5 77.5T480-80Zm0-80q116 0 198-82t82-198q0-116-82-198t-198-82q-116 0-198 82t-82 198q0 116 82 198t198 82Zm0-280Z"/></svg>
                <div class="time-box">
                    <div id="zegarek"></div>
                    <div id="data"></div> 
                </div>
            </div>
    </nav>
';

// Edycja kategorii
function EdytujKategorie($id){
    global $link;

    
    $query = "SELECT * FROM categories WHERE id = ?";
    $stmt = $link->prepare($query);
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $result = $stmt->get_result();

    $category = $result->fetch_assoc();

    // Formularz edycji kategorii
    echo '
        <h1>Edytuj kategorię</h1>
        <div class="edit-form-container">
            <form method="post" class="edit-form">
                <input type="hidden" name="id" value="' . htmlspecialchars($category['id']) . '">
                <label for="category_name">Nazwa kategorii:</label>
                <input type="text" name="category_name" id="category_name" value="' . htmlspecialchars($category['nazwa']) . '" required>

                <label for="mother_category">Matka:</label>
                <input type="number" name="mother_category" id="mother_category" value="' . htmlspecialchars($category['matka']) . '" required>

                <button type="submit" name="update">Zapisz Zmiany</button>
            </form>
        </div>
    ';

}

// Obsługa edycji kategorii

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_category'])) {
    EdytujKategorie(intval($_POST['id']));
}

// Edytowanie kategorii
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {

    $id = intval($_POST['id']);
    $category_name = $_POST['category_name'];
    $mother_category = $_POST['mother_category'];

    $query = "UPDATE categories SET nazwa = ?, matka = ? WHERE id = ?";
    $stmt = $link->prepare($query);
    $stmt->bind_param('sii', $category_name, $mother_category, $id);
    $stmt->execute();

    header("Location: " . $_SERVER['PHP_SELF']);
}

function PokazKategorie($parent_id = 0) {
    global $link; // Użycie globalnej zmiennej $link

    // Pobierz kategorie, które mają określony parent_id
    $stmt = mysqli_prepare($link, "SELECT * FROM categories WHERE matka = ? ORDER BY id ASC");
    mysqli_stmt_bind_param($stmt, "i", $parent_id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);

    // Tworzenie tabeli
    if (mysqli_num_rows($result) > 0) {
        echo '<table>';
        echo '<tr><th>ID</th><th>Nazwa</th><th>Matka</th><th>Akcje</th></tr>';
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<tr>
                    <td>' . $row['id'] . '</td>
                    <td>' . htmlspecialchars($row['nazwa']) . '</td>
                    <td>' . ($row['matka'] == 0 ? 'Brak (Główna kategoria)' : $row['matka']) . '</td>
                    <td>
                        <form method="post" style="display:inline;">
                            <input type="hidden" name="id" value="' . $row['id'] . '">
                            <button type="submit" name="edit_category" class="save">Edytuj</button>
                        </form>
                        <form method="post" style="display:inline;">
                            <input type="hidden" name="id" value="' . $row['id'] . '">
                            <button type="submit" name="delete_category" class="save">Usuń</button>
                        </form>
                        <form method="post" style="display:inline;">
                            <input type="hidden" name="parent_id" value="' . $row['id'] . '">
                            <button type="submit" name="view_subcategories" class="save">Zobacz podkategorie</button>
                        </form>
                    </td>
                  </tr>';
        }
        echo '</table>';
    } else {
        echo '<p>Brak kategorii do wyświetlenia.</p>';
    }

    mysqli_stmt_close($stmt); // Zamknięcie zapytania
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['view_subcategories'])) {
    $parent_id = intval($_POST['parent_id']);
    echo '<h1>Podkategorie</h1>';
    PokazKategorie($parent_id);
    echo '<form method="post" style="margin-top: 10px;">
            <button type="submit" name="back_to_main_categories" class="save">Powrót do głównych kategorii</button>
          </form>';
} elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['back_to_main_categories'])) {
    echo '<h1>Główne kategorie</h1>';
    PokazKategorie(); // Domyślnie pokaż główne kategorie
} else {
    echo '<h1>Główne kategorie</h1>';
    PokazKategorie(); // Domyślnie pokaż główne kategorie
}

function DodajNowaKategorie() {
    global $link;

    // Obsługa dodawania nowej kategorii
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_category'])) {
        $category_name = trim($_POST['category_name']);
        $mother_category = intval($_POST['mother_category']); // Zapewnienie, że jest liczbą

        // Walidacja pól formularza
        if (!empty($category_name) && $mother_category >= 0) {
            $query = "INSERT INTO categories (nazwa, matka) VALUES (?, ?)";
            $stmt = $link->prepare($query);

            if ($stmt) {
                $stmt->bind_param('si', $category_name, $mother_category);
                $stmt->execute();
            }

            // Przekierowanie po dodaniu (bez względu na wynik)
            header("Location: " . $_SERVER['PHP_SELF']);
            exit; // Zatrzymanie dalszego wykonywania kodu
        }
    }
}

// Formularz dodania nowej kategorii
echo '
    <div class="edit-form-container">
    <form method="post" class="add-page-form">
        <h2>Dodaj nową kategorię</h2>

        <label for="category_name">Nazwa kategorii:</label>
        <input type="text" name="category_name" id="category_name" required>

        <label for="mother_category">Matka:</label>
        <input type="number" name="mother_category" id="mother_category" min="0" required>

        <button type="submit" name="add_category">Dodaj kategorię</button>
    </form>
</div>
';

//Funckja usuwania kategorii
function UsunPodstrone($id) {
    global $link;

    //Przygotowanie zapytania
    $query = "DELETE FROM categories WHERE id = ?";
    $stmt = $link->prepare($query);
    $stmt->bind_param('i', $id);
    //Wykonanie zapytania
    $stmt->execute();

    //Zamknięcie zapytania
    $stmt->close();

    //Przekierowanie na tą samą stronę
    header("Location: " . $_SERVER['PHP_SELF']);
    exit;
}

//Obsługa usuwania kategorii
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_category'])) {
    UsunPodstrone(intval($_POST['id']));
}




DodajNowaKategorie();// wywołanie funkcji dodawania kategorii







ob_end_flush(); // Wysłanie bufora wyjścia
?>